	   ...
	   .ToList();
		...
		public async Task<IActionResult> Editar(int id, Candidato candidato, IFormFile Imagen)
		{
			...
			try
			{
				...
				{
					TempData["AlertMessage"] = "No se encontró el candidato.";
					return RedirectToAction("ListadoCandidato");
				}
				...
				candidatoExistente.Eleccion = await _context.Elecciones.FindAsync(candidato.IdEleccion);
				...
				if (Imagen != null && Imagen.Length > 0)
				{
					...
					{
						string urlImagen = await _ServicioImagen.SubirImagen(image, Imagen.FileName);
						candidatoExistente.ImgCandidato = urlImagen;
					}
				}

				// Marcar la entidad como modificada
				_context.Entry(candidatoExistente).State = EntityState.Modified;
